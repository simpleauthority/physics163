...
class CoulombsLawSimulationRewritten(Simulation):
    def __init__(self):
        super().__init__(auto_run=True, paused=False, log_level=LogLevel.EVERYTHING)

    def create_sim_data(self):
        d = Convert.to_base(1, original_unit=Unit.NANO)
        x_min = -4 * d

        return {
            'x_step': 0.3e-11,  # how much to step the loop per iteration
            'x_min': x_min,  # x min position
            'x_max': 6 * d,  # x max position
            'x': x_min,  # initial x position
            'dist': d,  # distance of each statically positioned charge from origin
            'e': 1.602e-19,  # e; magnitude of electron charge
        }

    def create_objects(self):
        charge1 = ElectricCharge(value=-1 * self.E, pos=vec(self.D, 0, 0), name="q1", value_alias="-e", draw=True,
                                 object_props=dict(radius=0.20 * self.D, color=color.cyan))

        charge2 = ElectricCharge(value=2 * self.E, pos=vec(-self.D, 0, 0), name="q2", value_alias="2e", draw=True,
                                 object_props=dict(radius=0.24 * self.D, color=color.red))

        charge3 = ElectricCharge(value=-1 * self.E, pos=vec(self.x, 0, 0), name="q3", value_alias="e", draw=True,
                                 object_props=dict(radius=0.15 * self.D, color=vec(0.8, 0, 1),
                                                   opacity=0.8))

        force13 = ElectricForce(q1=self.charge1, q2=self.charge3, draw=True,
        ...
        force23 = ElectricForce(q1=self.charge2, q2=self.charge3, draw=True,
                                base_scale_factor=global_arrow_scale_factor, trace_log=self.trace_log,
...
sim = CoulombsLawSimulationRewritten()
sim.start_sim()
