...
scene.title = "<h2>Map of Electric Potential and Field Arrows</h2><p>Simulates a map of electric potential and field arrows as caused by two point charges in space.</p><br />"
...
print("Drawing the electric potential grids and field arrows...")
...
for x in arange(x_min, x_max + dx, dx):
    for y in arange(y_min, y_max + dy, dy):
        # print(f"Drawing potential grid and field arrow at ({x}, {y})")
        rate(240)
        ...
        if mag(r1) == 0 or mag(r2) == 0:
            # if the POI is directly on top of a charge, both the potential and the field become infinite - we ignore them
            E = vec(0, 0, 0)
            V = 0
        else:
...
print("The electric potential grids and field arrows have been drawn.")
