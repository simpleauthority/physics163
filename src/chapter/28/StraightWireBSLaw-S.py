...
num_slices = 40  # how many slices to cut wire into
...
point_of_interest = vec(0, 0, -1)  # point at which to calculate B field
...
if mag(desaturated_b_experimental) == 0:
    ...
    B_total_arrow = arrow(pos=point_of_interest, axis=desaturated_b_experimental * scale_factor(desaturated_b_experimental), color=color.blue)
...
if B_theory == 0:
    ...
    print(f"% diff from theory = {(((mag(B_experimental) - B_theory) / B_theory) * 100):.1f}%")
