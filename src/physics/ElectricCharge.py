...
class ElectricCharge:
    ...
    def draw_obj(self):
        if self.object is None:
            self.object = sphere(pos=self.position, **self.object_props)
    ...
    def draw_label(self):
        if self.label is None:
            self.label = label(pos=self.position + vec(0, -2 * self.object.radius, 0), height=11, text=self.get_label_text())

    # Updates the label pos to the object position
    def tick_label_pos(self):
        if self.label is not None:
            self.label.position = self.object.position + vec(0, -2 * self.object.radius, 0)
